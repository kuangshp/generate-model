...
func (t *Table2Struct) Run() error {
	...
	for tableRealName, item := range tableColumns {
		...
		//}
		structName = t.camelCase(structName)
		if t.config.StructEnd != "" {
			structName = fmt.Sprintf("%s%s", structName, t.config.StructEnd)
		}
	...
	for rows.Next() {
		...
		if t.config.RmTagIfUcFirsted &&
			...
			if t.config.JsonTagToHump {
				...
				jsonTag = t.lowerCamelCase(jsonTag)
			}
...
// lowerCamelCase 转换为小驼峰(针对数据库字段)
func (t *Table2Struct) lowerCamelCase(str string) string {
	str = strings.Replace(str, "_", " ", -1) // 根据_来替换成
	str = strings.Title(str)                 // 全部大写
	str = strings.Replace(str, " ", "", -1)  // 删除空格
